function DisplayMonsterResult(actions, html) {
    if (GLOBAL_SETTINGS_OBJECT.DndBeyond_SlackEnabled) {
        ...
        var fd = new FormData();
        fd.append("token", GLOBAL_SETTINGS_OBJECT.DndBeyond_SlackToken);
        fd.append("channel", GLOBAL_SETTINGS_OBJECT.DndBeyond_SlackChannel);
...
function ExecuteActions(actions) {
    ...
    for (var i = 0, len = actions.length; i < len; i++) {
        if (html) {
            html = html + "<br />" + ExecuteAction(actions[i]);
        } else {
            html = html + ExecuteAction(actions[i]);
        }
    }
...
function ExecuteAction(action) {
    ...
    if (typeof action.MainRoll != 'undefined') {
        if (action.MainRoll.Dice > 0) {
            var rolledDice = RollDice(action.MainRoll, true);

            expression = BuildExpression(expression, rolledDice.Expression);
    ...
    if (typeof action.MainRoll.LinkedRolls != 'undefined') {
        ...
        for (var i = 0, len = action.MainRoll.LinkedRolls.length; i < len; i++) {
            ...
            var rolledDice = RollDice(action.MainRoll.LinkedRolls[i], action.MainRoll.LinkedRolls.length > 1);
...
function RollDice(roll, isPartOfChain) {
    var rolls = "";
    var totalValue = 0;
    var status = "";
    var savingThrowText = "";
    ...
    //Finalize data
    rolls = rolls + " " + FormatNumber(roll.Modifier);
    ...
    return {
        ...
        Expression: roll.Dice + "d" + roll.Sides + " " + FormatNumber(roll.Modifier),
        Status: status,
        TotalValue: totalValue
    };
}

function BuildExpression(exp, value) {
    if (exp) {
        exp = exp + " | " + value;
    } else {
        exp = value;
