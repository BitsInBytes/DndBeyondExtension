﻿function loadCharacterData() {
	$(".ct-character-tidbits").waitUntilExists(function () {
		GLOBAL_CHARACTER_OBJECT = { exists: true };
	...
	$(".ct-reset-pane__hitdie").waitUntilExists(function () {
		...
		hitDie.find(".ct-slot-manager__slot").click(function () {
			...
			createLinkedRolls(hitPointDice, "Healing", mainAction);

			ExecuteActions([mainAction]);
		});

		//loadInDie(init.find('.ct-signed-number__sign'));
	});
	...
	$(".ct-spells-spell").waitUntilExists(function () {
		...
		if (damageElement.length) {
			...
			if (damageValueElement.length) {
				...
				createLinkedRolls(damageText, damageType, mainAction);
			}
			...
			if (healingValueElement.length) {
				...
				createLinkedRolls(healingText, healingType, mainAction);
			}
		}
	...
	//					createLinkedRolls(damageText, damageType, newMainAction);
	//				}
	//			}
	...
	$(".ct-combat-attack").waitUntilExists(function () {
		...
		if (damageRows.length) {
			damageRows.each(function () {
				...
				MakeElementClickable(damageValueElement);
...
function createLinkedRolls(damageAmount, damageType, mainAction) {
    var damageParts = [];
    var modifier = 0;
